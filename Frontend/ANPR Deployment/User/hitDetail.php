...
if (isset($_GET['id'])) {
    ...
    $query = "SELECT records.*,camera.*, records.location AS record_location, hitlogs.id AS hitlog_id, hitlogs.created_at AS hit_timeStamp FROM hitlogs INNER JOIN bolo ON hitlogs.record_id = bolo.id INNER JOIN records ON bolo.record_id = records.Id INNER JOIN camera ON hitlogs.camera_id = camera.id WHERE hitlogs.id = $id";
...
if (isset($_GET['Id']) && isset($_GET['Plate']) && isset($_GET['action']) && $_GET['action'] == "close"){
    ...
    $hitlogs_query = mysqli_query($conn,"DELETE hitlogs FROM hitlogs INNER JOIN bolo ON hitlogs.record_id = bolo.id INNER JOIN records ON bolo.record_id = records.Id WHERE plate_no='$Plate'");
                                ...
                                <div class="card-body">
                                    <?php
                                        if ($result->num_rows > 0) {
                                            ...
                                            echo "<img src='../Snapshots/" . $id . "_frame.jpg' alt='Snapshot' style='max-width: 700px; height: auto; border-radius: 6px;' />";
                                            echo "</a>";
                                            ...
                                            echo "<div class='row'>";
                                                echo "<div class='col-md-3'>";
                                                    ...
                                                    echo "<p><strong>Time Stamp:</strong> " . $row['hit_timeStamp'] . "</p>";
                                            ...
                                            echo "<div class='row'>";
                                                echo "<div class='col-md-6'>";
                                                    echo "<a href='?action=close&Id=".$row['hitlog_id']."&Plate=".$row['plate_no']."' class='btn btn-danger' >Close BOLO</a>";
                                                echo "</div>";
                                                echo "<div class='col-md-6'>";
                                                    echo "<a href='?action=delete&Id=".$row['hitlog_id']."' class='btn btn-warning' style='float: right;'>Ignore</a>";
                                                echo "</div>";
                                            echo "</div>";
                                        }
                                        else{
